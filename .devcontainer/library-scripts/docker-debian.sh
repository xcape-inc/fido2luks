...
if [ "${USE_MOBY}" = "true" ]; then
    ...
    curl -fSL ${MICROSOFT_GPG_KEYS_URI} | gpg --dearmor > /usr/share/keyrings/microsoft-archive-keyring.gpg
    ...
    curl -fSL https://download.docker.com/linux/${ID}/gpg | gpg --dearmor > /usr/share/keyrings/docker-archive-keyring.gpg
...
if type docker-compose > /dev/null 2>&1; then
    ...
    if [ "${TARGET_COMPOSE_ARCH}" != "x86_64" ]; then
        ...
        curl -fSL "https://github.com/docker/compose/releases/download/${compose_v1_version}/docker-compose-Linux-x86_64" -o /usr/local/bin/docker-compose
...
if ! type compose-switch > /dev/null 2>&1; then
    ...
    curl -fSL "https://github.com/docker/compose-switch/releases/download/v${compose_switch_version}/docker-compose-linux-${architecture}" -o /usr/local/bin/compose-switch
